...
print(f"Total Loan Cost = {total_loan_cost}")
...
print(f"Average Loan Amount = {round_down_two_decimals(average_loan_amount)}")
...
print(f"Present Value = {round_down_two_decimals(present_value)}")
...
if present_value >= loan.get("loan_price"):
    print("Yes, Buy! This Loan is worth at least the cost to buy it")
else:
...
def calculate_present_value(future_value, remaining_months, annual_discount_rate):
    # convert discount rate from percentage value to decimal value, if required
    if annual_discount_rate > 1 :
        annual_discount_rate = annual_discount_rate / 100
...
with open("inexpensive_loans.csv", 'w') as csvfile:
    ...
    csvwriter.writerow(header)  # writing header row
    for loan in inexpensive_loans :
        csvwriter.writerow(loan.values())  # writing comma-separated values of inexpensive loans one row at a time
    
csvfile.close()  # closing file after we're done
